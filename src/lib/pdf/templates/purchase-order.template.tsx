...
import type { PurchaseOrderSnapshot } from "../../../modules/invoicing/invoicing.types";
...
const styles = StyleSheet.create({
    ...
    col: { flexDirection: "column", maxWidth: "31%" },
...
export function PurchaseOrderDocument({ snapshot }: { snapshot: PurchaseOrderSnapshot }) {
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                <Text style={styles.title}>Purchase Order</Text>
                <Text style={styles.subtitle}>
                    PO No: {snapshot.poNumber}  |  Order: {snapshot.orderDisplayId ?? snapshot.orderId}  |  Placed:{" "}
                    {new Date(snapshot.orderPlacedAt).toLocaleDateString("en-IN")}  |  Issued:{" "}
                    {new Date(snapshot.issuedAt).toLocaleDateString("en-IN")}
                </Text>
                ...
                <View style={styles.row}>
                    ...
                    {snapshot.shippingAddress ? (
                        <View style={styles.col}>
                            ...
                            <Text style={styles.partyName}>{snapshot.shippingAddress.receiverName}</Text>
                            <Text style={styles.partyLine}>
                                {snapshot.shippingAddress.street}, {snapshot.shippingAddress.city}
                            </Text>
                            <Text style={styles.partyLine}>
                                {snapshot.shippingAddress.state} - {snapshot.shippingAddress.pincode}
                            </Text>
                            <Text style={styles.partyLine}>{snapshot.shippingAddress.phone}</Text>
                        </View>
                    ) : null}
                </View>
                ...
                <View style={styles.totals}>
                    ...
                    <View style={styles.grandTotalRow}>
                        <Text>Agreed Total</Text>
                        <Text>{formatMoney(snapshot.grandTotal)}</Text>
                    </View>
                </View>

                <Text style={styles.footer}>
                    This purchase order records the terms agreed at the time order{" "}
                    {snapshot.orderDisplayId ?? snapshot.orderId} was placed/confirmed. It is not a tax invoice.
                    Buyer contact details shown are the account holder's; this platform does not yet capture a
                    separate business billing address, so the shipping address above may not match the buyer's
                    registered billing address for organizational purchases.
                </Text>
            </Page>
        </Document>
    );
